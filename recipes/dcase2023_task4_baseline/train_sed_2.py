...
def single_run(
    ...
    desed_test = TestModel(mel_spec = desed_training.mel_spec,
                           sed_student = desed_training.sed_student, 
                           sed_teacher = desed_training.sed_teacher, 
                           scaler = desed_training.scaler, 
                           supervised_loss = desed_training.supervised_loss, 
                           hparams = desed_training.hparams, 
                           encoder = desed_training.encoder,
                           evaluation = evaluation,
                           log = desed_training.log,
                           fast_dev_run=fast_dev_run,
                           exp_dir = desed_training.exp_dir)
    desed_test.test_start_tracking()
    ...
    for batch in test_loader:
        desed_test.test_step(batch) 
    desed_test.on_test_epoch_end()
...
def prepare_run(argv=None):
    ...
    if test_from_checkpoint is not None:
        checkpoint = torch.load(test_from_checkpoint, map_location=torch.device('cpu'))
