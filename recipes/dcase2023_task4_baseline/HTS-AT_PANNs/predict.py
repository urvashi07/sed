...
if __name__ == "__main__":
    ...
    test_dataset = Subset(test_dataset, np.arange(5))
    print(test_dataset["audio_name"])
    ...
    if args.model == "hts-at" or args.model == "htsat":
        ...
        ckpt_path = "/work/unegi2s/new_results/epochs_25/student_teacher/mixup_specaugment/logs/hts-at/checkpoints/epoch=19-step=10520.ckpt"
    ...
    clip_output, frame_output = trainer.predict_step(model, test_dataloader)
    h5py_file.close()
    end_time = time.time()
    ...
    target_int = [index for index, value in enumerate(test_dataset["target"][0]) if value == 1] 
    target_classes = [config.id2classes.get(integer, "Unknown") for integer in target_int]
    greater_than_0_5 = [1 if output > 0.5 else 0 for output in clip_output[0] ]
    output_int = [index for index, value in enumerate(greater_than_0_5) if value == 1]
    output_classes = [config.id2classes.get(integer, "Unknown") for integer in output_int]
    # Load audio signal first
    audio_container = dcase_util.containers.AudioContainer().load(
    test_dataset["waveform"][0]
    )
