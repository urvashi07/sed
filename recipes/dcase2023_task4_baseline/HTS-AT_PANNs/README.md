...
The code has been modified to take into account both the models: PANNs and HTS-AT (transformer based) for easier switching and evaluation.

Pretrained models can be downloaded from:
* [PANNs](https://zenodo.org/record/3987831) and place in the same directory (`HTS-AT_PANNs`)
* [HTS-AT](https://drive.google.com/drive/folders/1f5VYMk0uos_YnuBshgmaTVioXbs7Kmz6?usp=sharing) and place in folder ckpt (`HTS-AT_PANNs/ckpt`) or change the pretrained checkpoint directory in `config.py`

Data needs to be placed under `dcase2023_task4_baseline/DESED_task/data` (the data folder should contain the audio and labels)

Config file for the training parameters and HTS-AT model related parameters are available in `config.py`

For the audio directory file directory and paths of labelled files, it is available at `confs/default.yaml.`
These config parameters will be merged soon in one file.

### Run the code by running:
python `main_panns_hts-at.py [model_name]`    

model_name can be:
* panns
* hts-at

Eg: to run with hts-at model, the code can be run by:   

`python main_panns_hts-at.py hts-at`

TO DO:
* Combine config.py and config parameters under `confs/default.yaml`
* Use strongly labelled data
* Code refactoring
* Add CRNN model
* Try BEATs and/or OpenL3 embeddings
* Data Augmentation
