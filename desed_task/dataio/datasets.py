...
class StronglyAnnotatedSet(Dataset):
    def __init__(
        ...
        for i, r in tsv_entries.iterrows():
            if not os.path.isfile(os.path.join(audio_folder, r["filename"])):
                missing_counter+=1
                continue
        ...
        num_events = sum(
            [1 for file_name,d in examples.items() for _ in d["events"]]
        )
        print(
            f"{self.__class__.__name__}, {audio_folder} got {num_events} events, {missing_counter} are missing"
        )
